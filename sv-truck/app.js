...
(async () => {
    ...
    const consumer = kafka.consumer({ groupId: 'sv-truck-group' });

    // create app domain
    const domain = new AppDomain(mongoClient.db("sv-truck"), producer);
    ...
    app.get("/api/truck/v1/list", async (req, res) => {
    ...
    consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
            ...
            if (topic === 'order-created') {
                await domain.assignTruckForOrder(JSON.parse(message.value));
            } else if (topic === 'order-cancelled') {
                await domain.unassignTruckForOrderCancellation(JSON.parse(message.value));
            } else {
